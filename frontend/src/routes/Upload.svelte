<script>
     ...
     let isUploading = false, isImageInBlob = false, hasPressedButton = false, error = null;
     ...
     async function sendImage(blobUrl) {
          ...
          try {
               ...
               if (!response.ok) {
                    ...
                    error = { message: errorData.message || "Upload failed", code: response.status };
               } else {
                    json = await response.json();
                    console.log(json);
               }
          } catch (err) {
               error = { message: err.message || "Network error", code: "" };
          }
...
<section>
     ...
     {:else if hasPressedButton && error}
          <Error message={error.message} code={error.code}/>
     {:else if hasPressedButton && json}
